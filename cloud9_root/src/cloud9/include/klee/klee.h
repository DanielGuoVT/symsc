...
// Daniel, for tapPLC
void klee_task_priority(uint64_t tid, uint64_t priority);
void klee_task_period(uint64_t tid, uint64_t period);

void klee_scan_start();
void klee_scan_end();
void klee_task_boundary_start();
void klee_task_boundary();
void klee_task_boundary_end();
...
void klee_mutex_unlock_end(uint64_t mutex);
