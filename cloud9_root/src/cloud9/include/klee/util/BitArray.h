...
public:
  ...
  void set(unsigned idx) { bits[idx/32] |= 1<<(idx&0x1F); } // set the bit at the k-th position in the array
  void unset(unsigned idx) { bits[idx/32] &= ~(1<<(idx&0x1F)); } // clear the k-th bit in the bitarray
