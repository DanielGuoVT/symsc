...
void testOperation(Solver &solver,
  ...
  klee::ref<Expr> fullySymbolicExpr = Expr::createFromKind(T::kind, symbolicArgs);
  ...
  for (unsigned kid = 0; kid < T::numKids; kid++) {
    ...
    klee::ref<Expr> partiallyConstantExpr =
      Expr::createFromKind(T::kind, partiallyConstantArgs);
    
    klee::ref<Expr> queryExpr = EqExpr::create(fullySymbolicExpr, 
