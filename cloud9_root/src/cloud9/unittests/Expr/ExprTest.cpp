...
klee::ref<Expr> getConstant(int value, Expr::Width width) {
  int64_t ext = value;
...
TEST(ExprTest, BasicConstruction) {
  EXPECT_EQ(klee::ref<Expr>(ConstantExpr::alloc(0, 32)),
...
TEST(ExprTest, ConcatExtract) {
  ...
  klee::ref<Expr> read8 = Expr::createTempRead(array, 8);
  Array *array2 = new Array("arr1", 256);
  klee::ref<Expr> read8_2 = Expr::createTempRead(array2, 8);
  klee::ref<Expr> c100 = getConstant(100, 8);

  klee::ref<Expr> concat1 = ConcatExpr::create4(read8, read8, c100, read8_2);
  EXPECT_EQ(2U, concat1->getNumKids());
  EXPECT_EQ(2U, concat1->getKid(1)->getNumKids());
  EXPECT_EQ(2U, concat1->getKid(1)->getKid(1)->getNumKids());

  klee::ref<Expr> extract1 = ExtractExpr::create(concat1, 8, 16);
  EXPECT_EQ(Expr::Concat, extract1->getKind());
  EXPECT_EQ(read8, extract1->getKid(0));
  EXPECT_EQ(c100, extract1->getKid(1));

  klee::ref<Expr> extract2 = ExtractExpr::create(concat1, 6, 26);
  ...
  klee::ref<Expr> extract3 = ExtractExpr::create(concat1, 24, 1);
  EXPECT_EQ(Expr::Extract, extract3->getKind());

  klee::ref<Expr> extract4 = ExtractExpr::create(concat1, 27, 2);
  ...
  klee::ref<Expr> extract5 = ExtractExpr::create(concat1, 17, 5);
  EXPECT_EQ(Expr::Extract, extract5->getKind());

  klee::ref<Expr> extract6 = ExtractExpr::create(concat1, 3, 26);
  ...
  klee::ref<Expr> concat10 = ConcatExpr::create4(read8, c100, c100, read8);    
  klee::ref<Expr> extract10 = ExtractExpr::create(concat10, 8, 16);
  EXPECT_EQ(Expr::Constant, extract10->getKind());
}

TEST(ExprTest, ExtractConcat) {
  Array *array = new Array("arr2", 256);
  klee::ref<Expr> read64 = Expr::createTempRead(array, 64);

  Array *array2 = new Array("arr3", 256);
  klee::ref<Expr> read8_2 = Expr::createTempRead(array2, 8);
  
  klee::ref<Expr> extract1 = ExtractExpr::create(read64, 36, 4);
  klee::ref<Expr> extract2 = ExtractExpr::create(read64, 32, 4);
  
  klee::ref<Expr> extract3 = ExtractExpr::create(read64, 12, 3);
  klee::ref<Expr> extract4 = ExtractExpr::create(read64, 10, 2);
  klee::ref<Expr> extract5 = ExtractExpr::create(read64, 2, 8);
   
  klee::ref<Expr> kids1[6] = { extract1, extract2,
       read8_2,
       extract3, extract4, extract5 };
  klee::ref<Expr> concat1 = ConcatExpr::createN(6, kids1);
  EXPECT_EQ(29U, concat1->getWidth());
  
  klee::ref<Expr> extract6 = ExtractExpr::create(read8_2, 2, 5);
  klee::ref<Expr> extract7 = ExtractExpr::create(read8_2, 1, 1);
  
  klee::ref<Expr> kids2[3] = { extract1, extract6, extract7 };
  klee::ref<Expr> concat2 = ConcatExpr::createN(3, kids2);
...
TEST(ExprTest, TrueFalseExpr) {
  ...
  klee::ref<Expr> trueExpr = exprBuilder->True();
