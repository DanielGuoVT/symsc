...
static cl::opt<bool> DisableInline("disable-inlining", cl::desc("Do not run the inliner pass"), cl::init(true));

static cl::opt<bool> DisableOptimizations("disable-opt", cl::desc("Do not run any optimization passes"), cl::init(false));
...
static void AddStandardCompilePasses(PassManager &PM) {
	...
	if (StripDebug) addPass(PM, createStripSymbolsPass(true));
...
void Optimize(Module* M) {
	...
	if (!DisableOptimizations) {
		...
		// internal.
		if (!DisableInternalize) addPass(Passes, createInternalizePass(true));
		...
		if (!DisableInline) addPass(Passes, createFunctionInliningPass()); // Inline small functions
	...
	if (Strip || StripDebug) addPass(Passes, createStripSymbolsPass(StripDebug && !Strip));
...
//	addPass(Passes, createInstIdPass()); //pass for adding clap_id
//	addPass(Passes, createInstructionNamerPass());
