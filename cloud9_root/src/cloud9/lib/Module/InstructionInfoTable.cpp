...
bool InstructionInfoTable::getInstructionDebugInfo(const llvm::Instruction *I, const std::string *&File, int &FileID, unsigned &Line) {
...
bool InstructionInfoTable::ltfunc::operator()(const Function *f1, const Function *f2) const {
...
InstructionInfoTable::InstructionInfoTable(Module *m) :
	...
	for (std::vector<Function*>::iterator fnIt = functions.begin(); fnIt != functions.end(); fnIt++) {
		Function *f = *fnIt;
		...
		typedef std::map<BasicBlock*, std::pair<std::pair<const std::string*, int>, unsigned> > sourceinfo_ty;
		...
		for (llvm::Function::iterator bbIt = f->begin(), bbie = f->end(); bbIt != bbie; ++bbIt) {
			std::pair<sourceinfo_ty::iterator, bool> res = sourceInfo.insert(
					std::make_pair(bbIt, std::make_pair(std::make_pair(initialFile, initialID), initialLine)));
			if (!res.second)
				continue;
			...
			do {
				...
				for (BasicBlock::iterator it = bb->begin(), ie = bb->end(); it != ie; ++it) {
					...
					infos.insert(std::make_pair(instr, InstructionInfo(id++, *file, fileID, line, assemblyLine)));
				}

				for (succ_iterator it = succ_begin(bb), ie = succ_end(bb); it != ie; ++it) {
					if (sourceInfo.insert(std::make_pair(*it, std::make_pair(std::make_pair(file, fileID), line))).second)
...
InstructionInfoTable::~InstructionInfoTable() {
	for (std::map<const std::string *, int, ltstr>::iterator it = stringTable.begin(), ie = stringTable.end(); it != ie; ++it)
		delete it->first;
}
...
InstructionInfoTable::getInfo(const Instruction *inst) const {
	InfoMap::const_iterator it = infos.find(inst);
	if (it == infos.end()) {
		return dummyInfo;
	} else {
		return it->second;
