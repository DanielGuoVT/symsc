...
TreeStreamWriter::TreeStreamWriter(const std::string &_path) :
		lastID(0), bufferCount(0), path(_path), output(new std::ofstream(path.c_str(), std::ios::out | std::ios::binary)), ids(1) {
	if (!output->good()) {
		delete output;
		output = 0;
	}
...
#if 1
	if (bufferCount && (os.id != lastID || size + bufferCount > bufferSize)) flushBuffer();
...
void TreeStreamWriter::readStream(TreeStreamID streamID, std::vector<unsigned char> &out) {
	...
	for (;;) {
		...
		if (tag & (1 << 31)) { // fork
			unsigned child = tag ^ (1 << 31);
			if (id == roots.back() && roots.size() > 1 && child == roots[roots.size() - 2]) roots.pop_back();
		} else {
			unsigned size = tag;
			if (id == roots.back()) {
				while (size--)
