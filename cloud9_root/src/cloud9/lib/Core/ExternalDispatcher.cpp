...
Function *ExternalDispatcher::createDispatcher(Function *target, Instruction *inst) {
	...
	Function *dispatcher = Function::Create(FunctionType::get(Type::getVoidTy(getGlobalContext()), ArrayRef<Type*>(), false),
			GlobalVariable::ExternalLinkage, "", dispatchModule);

	BasicBlock *dBB = BasicBlock::Create(getGlobalContext(), "entry", dispatcher);

	// Get a Value* for &gTheArgsP, as an i64**.
	Instruction *argI64sp = new IntToPtrInst(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), (uintptr_t) (void*) &gTheArgsP),
			PointerType::getUnqual(PointerType::getUnqual(Type::getInt64Ty(getGlobalContext()))), "argsp", dBB);
	...
	for (CallSite::arg_iterator ai = cs.arg_begin(), ae = cs.arg_end(); ai != ae; ++ai, ++i) {
		...
		Instruction *argI64p = GetElementPtrInst::Create(argI64s, ConstantInt::get(Type::getInt32Ty(getGlobalContext()), idx), "", dBB);
	...
	Constant *dispatchTarget = dispatchModule->getOrInsertFunction(target->getName(), FTy, target->getAttributes());
