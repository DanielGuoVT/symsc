...
namespace {
cl::opt<bool> TrackInstructionTime("track-instruction-time", cl::desc("Enable tracking of time for individual instructions"), cl::init(false));
...
cl::opt<bool> OutputDetailedCoverage("output-detailed-coverage", cl::desc("Write periodic detailed coverage information"), cl::init(true));

cl::opt<bool> OutputProfileSnapshots("output-profile-snapshots", cl::desc("Write periodic callgraph snapshots"), cl::init(false));

cl::opt<bool> OutputSolverQueries("output-solver-queries", cl::desc("Write the log of solver queries"), cl::init(true));

cl::opt<double> StatsWriteInterval("stats-write-interval", cl::desc("Approximate number of seconds between stats writes (default: 1.0)"),
		cl::init(1.));

cl::opt<double> IStatsWriteInterval("istats-write-interval", cl::desc("Approximate number of seconds between istats writes (default: 10.0)"),
		cl::init(10.));

cl::opt<double> DetailedLogsInterval("detailed-logs-interval", cl::desc("Approx. no. of secs between logging updates (default: 60.0)"),
		cl::init(60.0));

cl::opt<double> LogSummaryInterval("log-summary-interval", cl::desc("Approx. no. of secs between summary logs (default: 5.0)"), cl::init(5.0));
...
cl::opt<double> UncoveredUpdateInterval("uncovered-update-interval", cl::init(30.));

cl::opt<bool> UseCallPaths("use-call-paths", cl::desc("Enable calltree tracking for instruction level statistics"), cl::init(true));

cl::opt<std::string> CoverageFocus("coverage-focus", cl::desc("The name of a file containing source code to focus on "
...
class DetailedLogsTimer: public Executor::Timer {
	StatsTracker *statsTracker;
public:
	DetailedLogsTimer(StatsTracker *_statsTracker) :
			statsTracker(_statsTracker) {
	}
	~DetailedLogsTimer() {
	}
...
public:
	LogSummaryTimer(StatsTracker *_statsTracker) :
			statsTracker(_statsTracker) {
	}
	~LogSummaryTimer() {
	}
...
StatsTracker::StatsTracker(Executor &_executor, std::string _objectFilename, bool _updateMinDistToUncovered) :
		executor(_executor), objectFilename(_objectFilename), statsFile(0), istatsFile(0), detailedCoverageFile(0), profileFile(0), queriesFile(0), startWallTime(
				util::getWallTime()), numBranches(0), fullBranches(0), partialBranches(0) {
	...
	if (!llvm::sys::path::is_absolute(Twine(objectFilename))) {
		...
		if (current.isValid()) objectFilename = current.c_str();
	}

	if (OutputIStats) theStatisticManager->useIndexedStats(km->infos->getMaxID());

	for (std::vector<KFunction*>::iterator it = km->functions.begin(), ie = km->functions.end(); it != ie; ++it) {
		KFunction *kf = *it;
		//kf->trackCoverage = 1;
		if (kf->function->hasName()) {
			string name = kf->function->getName().str();
			if (executor.cov_functions.count(name)) {
...
StatsTracker::~StatsTracker() {
	...
	if (pstatsFile) delete pstatsFile;
...
void StatsTracker::stepInstruction(ExecutionState &es) {
	if (OutputIStats) {
		...
		if (UseCallPaths) theStatisticManager->setContext(&sf.callPathNode->statistics);

		if (es.instsSinceCovNew) ++es.instsSinceCovNew;

		if (instructionIsCoverable(inst)) {
			if (!theStatisticManager->getIndexedValue(stats::locallyCoveredInstructions, ii.id)) {
				// Checking for actual stoppoints avoids inconsistencies due
				// to line number propogation.
				es.coveredLines[&ii.file].insert(ii.line);
				es.setCoveredNew();
				es.instsSinceCovNew = 1;
				if (inst->getParent()->getParent()->hasName()) {
					string name = inst->getParent()->getParent()->getName().str();
					if (executor.cov_functions.count(name)) {
...
void StatsTracker::framePushed(StackFrame *frame, StackFrame *parentFrame) {
	if (OutputIStats) {
		...
		if (UseCallPaths) {
			...
			CallPathNode *cp = callPathManager.getCallPath(parent, frame->caller ? frame->caller->inst : 0, frame->kf->function);
			frame->callPathNode = cp;
			cp->count++;
		}

		uint64_t minDistAtRA = 0;
		if (parentFrame) minDistAtRA = parentFrame->minDistToUncoveredOnReturn;

		frame->minDistToUncoveredOnReturn = frame->caller ? computeMinDistToUncovered(frame->caller, minDistAtRA) : 0;
	}
}
...
void StatsTracker::markBranchVisited(ExecutionState *visitedTrue, ExecutionState *visitedFalse) {
	if (OutputIStats) {
		...
		if (visitedTrue && !hasTrue) {
			...
			if (visitedTrue->prevPC()->inst->getParent()->getParent()->hasName()) {
				string name = visitedTrue->prevPC()->inst->getParent()->getParent()->getName().str();
				if (executor.cov_functions.count(name)) {
					...
					if (hasFalse) {
						++fullBranches;
						--partialBranches;
//						std::cerr << std::endl<< "full: " <<visitedTrue->prevPC()->info->assemblyLine << std::endl;
//						visitedTrue->prevPC()->inst->dump();
					} else {
						++partialBranches;
//						std::cerr <<std::endl<< "partial true: " << visitedTrue->prevPC()->info->assemblyLine << std::endl;
//						visitedTrue->prevPC()->inst->dump();
					}
				}
			}
		...
		if (visitedFalse && !hasFalse) {
			...
			if (visitedFalse->prevPC()->inst->getParent()->getParent()->hasName()) {
				string name = visitedFalse->prevPC()->inst->getParent()->getParent()->getName().str();
				if (executor.cov_functions.count(name)) {
...
void StatsTracker::writeStatsHeader() {
	*statsFile << "('Instructions'," << "'FullBranches'," << "'PartialBranches'," << "'NumBranches'," << "'UserTime'," << "'NumStates',"
...
void StatsTracker::writeStatsLine() {
	*statsFile << "(" << stats::instructions << "," << fullBranches << "," << partialBranches << "," << numBranches << "," << util::getUserTime()
			<< "," << executor.states.size() << "," << sys::Process::GetTotalMemoryUsage() << "," << stats::queries << "," << stats::queryConstructs
			<< "," << 0
			// was numObjects
			<< "," << elapsed() << "," << stats::locallyCoveredInstructions << "," << stats::locallyUncoveredInstructions << ","
			<< stats::queryTime / 1000000. << "," << stats::solverTime / 1000000. << "," << stats::cexCacheTime / 1000000. << ","
...
void StatsTracker::updateStateStatistics(uint64_t addend) {
	for (std::set<ExecutionState*>::iterator it = executor.states.begin(), ie = executor.states.end(); it != ie; ++it) {
		ExecutionState &state = **it;
		const InstructionInfo &ii = *state.pc()->info;
		theStatisticManager->incrementIndexedValue(stats::states, ii.id, addend);
		if (UseCallPaths) state.stack().back().callPathNode->statistics.incrementValue(stats::states, addend);
	}
}
...
// Max is 13, sadly
	...
	of << "events: ";
	for (unsigned i = 0; i < nStats; i++) {
		if (istatsMask & (1 << i)) of << sm.getStatistic(i).getShortName() << " ";
	}
	of << "\n";

// set state counts, decremented after we process so that we don't
// have to zero all records each time.
	if (istatsMask & (1 << stats::states.getID())) updateStateStatistics(1);

	std::string sourceFile = "";

	CallSiteSummaryTable callSiteStats;
	if (UseCallPaths) callPathManager.getSummaryStatistics(callSiteStats);
		...
		if (!fnIt->isDeclaration()) {
			...
			for (Function::iterator bbIt = fnIt->begin(), bb_ie = fnIt->end(); bbIt != bb_ie; ++bbIt) {
				for (BasicBlock::iterator it = bbIt->begin(), ie = bbIt->end(); it != ie; ++it) {
					...
					of << ii.line << " ";
					for (unsigned i = 0; i < nStats; i++)
						if (istatsMask & (1 << i)) of << sm.getIndexedValue(sm.getStatistic(i), index) << " ";
					of << "\n";
					...
					if (UseCallPaths && (isa<CallInst>(instr) || isa<InvokeInst>(instr))) {
						...
						if (it != callSiteStats.end()) {
							for (std::map<llvm::Function*, CallSiteInfo>::iterator fit = it->second.begin(), fie = it->second.end(); fit != fie;
									++fit) {
								Function *f = fit->first;
								CallSiteInfo &csi = fit->second;
								const InstructionInfo &fii = executor.kmodule->infos->getFunctionInfo(f);

								if (fii.file != "" && fii.file != sourceFile) of << "cfl=" << fii.file << "\n";
	...
	if (istatsMask & (1 << stats::states.getID())) updateStateStatistics((uint64_t) -1);
...
uint64_t klee::computeMinDistToUncovered(const KInstruction *ki, uint64_t minDistAtRA) {
	...
	if (minDistAtRA == 0) { // unreachable on return, best is local
		return sm.getIndexedValue(stats::minDistToGloballyUncovered, ki->info->id);
	} else {
		uint64_t minDistLocal = sm.getIndexedValue(stats::minDistToGloballyUncovered, ki->info->id);
...
//	static bool init = true;
	...
	if (init) {
		...
		for (calltargets_ty::iterator it = callTargets.begin(), ie = callTargets.end(); it != ie; ++it)
			for (std::vector<Function*>::iterator fit = it->second.begin(), fie = it->second.end(); fit != fie; ++fit)
		...
		for (Module::iterator fnIt = m->begin(), fn_ie = m->end(); fnIt != fn_ie; ++fnIt) {
			...
			for (unsigned i = 0; i < kf->numInstructions; ++i) {
				...
				sm.setIndexedValue(stats::minDistToReturn, kf->instrPostOrder[i]->info->id, isa<ReturnInst>(inst));
			}
		}
		...
		do {
			changed = false;
			for (std::vector<Instruction*>::iterator it = instructions.begin(), ie = instructions.end(); it != ie; ++it) {
				Instruction *inst = *it;
				unsigned bestThrough = 0;

				if (isa<CallInst>(inst) || isa<InvokeInst>(inst)) {
					std::vector<Function*> &targets = callTargets[inst];
					for (std::vector<Function*>::iterator fnIt = targets.begin(), ie = targets.end(); fnIt != ie; ++fnIt) {
						uint64_t dist = functionShortestPath[*fnIt];
						if (dist) {
							dist = 1 + dist; // count instruction itself
							if (bestThrough == 0 || dist < bestThrough) bestThrough = dist;
						}
					}
				} else {
					bestThrough = 1;
				...
				if (bestThrough) {
					...
					for (std::vector<Instruction*>::iterator it2 = succs.begin(), ie = succs.end(); it2 != ie; ++it2) {
					...
					if (best != cur) {
						...
						if (inst == f->begin()->begin()) functionShortestPath[f] = best;
					}
				}
			}
		} while (changed);
...
// compute minDistToUncovered, 0 is unreachable
	...
	for (Module::iterator fnIt = m->begin(), fn_ie = m->end(); fnIt != fn_ie; ++fnIt) {
		...
		for (unsigned i = 0; i < kf->numInstructions; ++i) {
			...
			instructions.push_back(inst);
			ids.push_back(id);
			sm.setIndexedValue(stats::minDistToGloballyUncovered, id, sm.getIndexedValue(stats::globallyUncoveredInstructions, id));
		}
	}
...
// I'm so lazy it's not even worklisted.
	...
	do {
		...
		for (unsigned i = 0; i < instructions.size(); ++i) {
			...
			uint64_t best, cur = best = sm.getIndexedValue(stats::minDistToGloballyUncovered, id);
			unsigned bestThrough = 0;
			...
			if (isa<CallInst>(inst) || isa<InvokeInst>(inst)) {
				...
				for (std::vector<Function*>::iterator fnIt = targets.begin(), ie = targets.end(); fnIt != ie; ++fnIt) {
					...
					if (!(*fnIt)->isDeclaration()) {
						uint64_t calleeDist = sm.getIndexedValue(stats::minDistToGloballyUncovered, infos.getFunctionInfo(*fnIt).id);
						if (calleeDist) {
							calleeDist = 1 + calleeDist; // count instruction itself
							if (best == 0 || calleeDist < best) best = calleeDist;
						}
					}
				}
			} else {
			...
			if (bestThrough) {
				...
				for (std::vector<Instruction*>::iterator it2 = succs.begin(), ie = succs.end(); it2 != ie; ++it2) {
					uint64_t dist = sm.getIndexedValue(stats::minDistToGloballyUncovered, infos.getInfo(*it2).id);
					if (dist) {
						uint64_t val = bestThrough + dist;
						if (best == 0 || val < best) best = val;
					}
			...
			if (best != cur) {
				sm.setIndexedValue(stats::minDistToGloballyUncovered, infos.getInfo(inst).id, best);
				changed = true;
			}
		}
	} while (changed);
	...
	for (std::set<ExecutionState*>::iterator it = executor.states.begin(), ie = executor.states.end(); it != ie; ++it) {
		...
		for (ExecutionState::stack_ty::iterator sfIt = es->stack().begin(), sf_ie = es->stack().end(); sfIt != sf_ie; ++sfIt) {
...
void StatsTracker::computeDetailedCoverageSnapshot(data::CoverageInfo &coverageInfo, bool complete) {
	KModule *km = executor.kmodule;
	...
	for (std::vector<KFunction*>::iterator fit = km->functions.begin(), fie = km->functions.end(); fit != fie; fit++) {
		KFunction* kf = *fit;
		...
		for (unsigned i = 0; i < kf->numInstructions; i++) {
			if (!instructionIsCoverable(kf->instructions[i]->inst)) continue;
			unsigned id = kf->instructions[i]->info->id;
			if (!complete && !theStatisticManager->getIndexedValue(stats::globallyCoveredInstructions, id)) continue;
			...
			if (complete) {
				...
				if (coverageFocus.count(file_id) > 0) lastFocusedCoverage.total_asm_lines++;
			} else {
				lastGlobalCoverage.covered_asm_lines++;
				if (coverageFocus.count(file_id) > 0) lastFocusedCoverage.covered_asm_lines++;
			}
		}

		for (src_coverage_t::iterator cit = source_coverage.begin(), cie = source_coverage.end(); cit != cie; ++cit) {
			data::SourceFileCoverageInfo *srcCovInfo = fnCovInfo->add_source_coverage();
			srcCovInfo->set_file_id(cit->first);
			for (std::set<int>::iterator lit = cit->second.begin(), lie = cit->second.end(); lit != lie; ++lit) {
				srcCovInfo->add_covered_line(*lit);
			}
		}
	}

	for (src_coverage_t::iterator cit = global_source_coverage.begin(), cie = global_source_coverage.end(); cit != cie; ++cit) {
		if (complete) {
			lastGlobalCoverage.total_source_lines += cit->second.size();
			if (coverageFocus.count(cit->first) > 0) lastFocusedCoverage.total_source_lines += cit->second.size();
		} else {
			lastGlobalCoverage.covered_source_lines += cit->second.size();
			if (coverageFocus.count(cit->first) > 0) lastFocusedCoverage.covered_source_lines += cit->second.size();
		}
	}
...
void StatsTracker::writeSolverQueries() {
	if (!currentQuerySet.solver_query_size()) return;
...
void StatsTracker::writeStatistics() {
	if (!currentStatisticsSet.statistic_size()) return;
...
void StatsTracker::writeStates() {
	...
	if (!currentStatesSet.execution_state_size()) return;
	...
	for (Module::iterator fnIt = m->begin(), fn_ie = m->end(); fnIt != fn_ie; ++fnIt) {
		...
		for (Function::iterator bbIt = fnIt->begin(), bb_ie = fnIt->end(); bbIt != bb_ie; ++bbIt) {
			for (BasicBlock::iterator it = bbIt->begin(), ie = bbIt->end(); it != ie; ++it) {
				...
				executor.kmodule->fillInstructionDebugInfo(instr, *lineProfile->mutable_debug_info());

				for (unsigned i = 0; i < nStats; i++) {
					if (istatsMask & (1 << i)) {
						lineProfile->add_cost_value(sm.getIndexedValue(sm.getStatistic(i), index));
					}
				}
				...
				if (UseCallPaths && (isa<CallInst>(instr) || isa<InvokeInst>(instr))) {
					CallSiteSummaryTable::iterator it = callSiteStats.find(instr);
					if (it != callSiteStats.end()) {
						for (std::map<llvm::Function*, CallSiteInfo>::iterator fit = it->second.begin(), fie = it->second.end(); fit != fie; ++fit) {
							Function *f = fit->first;
							CallSiteInfo &csi = fit->second;

							data::CallSiteProfile *callsiteProfile = lineProfile->add_call_site_profile();
							executor.kmodule->fillFunctionDebugInfo(f, *callsiteProfile->mutable_debug_info());
...
void StatsTracker::readCoverageFocus(std::istream &is) {
	...
	while (!is.eof()) {
		...
		if (is.eof() && file_name.length() == 0) break;

		for (StringTable::iterator it = km->infos->stringTable.begin(), ie = km->infos->stringTable.end(); it != ie; ++it) {
			const std::string *str = it->first;
			if (str->rfind(file_name) != std::string::npos) coverageFocus.insert(it->second);
		}
	}
}
...
void StatsTracker::logSummary() {
	LOG(INFO) << "[STATS] StateCount: " << executor.states.size() << " InstructionCount: " << stats::instructions << " QueryCount: "
			<< stats::queries;

	LOG(INFO) << "[GLOBAL COVERAGE] " << "Source: "
			<< formatCoverageInfo(lastGlobalCoverage.total_source_lines, lastGlobalCoverage.covered_source_lines) << " Instructions: "
			<< formatCoverageInfo(lastGlobalCoverage.total_asm_lines, lastGlobalCoverage.covered_asm_lines);
	LOG(INFO) << "[FOCUSED COVERAGE] " << "Source: "
			<< formatCoverageInfo(lastFocusedCoverage.total_source_lines, lastFocusedCoverage.covered_source_lines) << " Instructions: "
			<< formatCoverageInfo(lastFocusedCoverage.total_asm_lines, lastFocusedCoverage.covered_asm_lines);
}

void StatsTracker::recordSolverQuery(uint64_t time_stamp, uint64_t solving_time, data::QueryReason reason, data::QueryOperation operation,
	...
	executor.kmodule->fillInstructionDebugInfo(ki->inst, *query->mutable_debug_info());
...
void StatsTracker::updateStates(ExecutionState *current, const std::set<ExecutionState*> &addedStates,
...
void StatsTracker::recordStateUpdate(const ExecutionState &state, bool terminated, bool set_stamp, data::ExecutionState *es_data) {
	if (set_stamp) es_data->set_time_stamp(sys::TimeValue::now().toPosixTime());
...
void StatsTracker::flushAllStates() {
	for (std::set<ExecutionState*>::iterator it = currentStates.begin(), ie = currentStates.end(); it != ie; ++it) {
		data::ExecutionState *es_data = currentStatesSet.add_execution_state();
