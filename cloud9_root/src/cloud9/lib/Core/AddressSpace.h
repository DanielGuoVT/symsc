...
public:
	AddressSpace() :
			cowKey(1), cowDomain(NULL) {
	}
	AddressSpace(const AddressSpace &b) :
			cowKey(b.cowKey), cowDomain(NULL), objects(b.objects) {
	}
	~AddressSpace() {
	}
	...
	bool resolveOne(const klee::ref<ConstantExpr> &address, ObjectPair &result);

	bool isGlobalAddress(const klee::ref<ConstantExpr> &address);
	...
	bool resolveOne(ExecutionState &state, TimingSolver *solver, klee::ref<Expr> address, ObjectPair &result, bool &success);
	...
	bool resolve(ExecutionState &state, TimingSolver *solver, klee::ref<Expr> address, ResolutionList &rl, unsigned maxResolutions = 0, double timeout = 0.);
	...
	void DumpMemoryObject(std::ostream &os, const MemoryObject *mo, const ObjectState *ostate, bool fast) const;
