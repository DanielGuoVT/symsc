...
ObjectState *AddressSpace::getWriteable(const MemoryObject *mo, const ObjectState *os) {
	assert(!os->readOnly);
	// here it is the real implementation of the copy-on-write
	if (cowKey != os->copyOnWriteOwner) {
		if (os->isShared) { // shared among processes.
			ObjectState *n = new ObjectState(*os);
			n->copyOnWriteOwner = cowKey;

			for (cow_domain_t::iterator it = cowDomain->begin(); it != cowDomain->end(); it++) {
				AddressSpace *as = *it;
...
bool AddressSpace::resolveOne(const klee::ref<ConstantExpr> &addr, ObjectPair &result) {
	...
	if (const MemoryMap::value_type *res = objects.lookup_previous(&hack)) {
		...
		if ((mo->size == 0 && address == mo->address) || (address - mo->address < mo->size)) {
			...
			// Daniel, capture the memory exception
			assert(false && "finding corresponding MemoryObject error !");
		}
	}
	if (VLOG_IS_ON(1)) {
...
/*
 * check if the given address is in the scope of a global variable
 */
bool AddressSpace::isGlobalAddress(const klee::ref<ConstantExpr> &addr) {
	uint64_t address = addr->getZExtValue();
	MemoryObject hack(address);
...
bool AddressSpace::resolveOne(ExecutionState &state, TimingSolver *solver, klee::ref<Expr> address, ObjectPair &result, bool &success) {
	if (ConstantExpr *CE = (dyn_cast<ConstantExpr>(address))) {
		...
		while (oi != begin) {
			...
			bool mayBeTrue;
			if (!solver->mayBeTrue(data::SINGLE_ADDRESS_RESOLUTION, state, mo->getBoundsCheckPointer(address), mayBeTrue))
				return false;
			if (mayBeTrue) {
				result = *oi;
				success = true;
				...
				bool mustBeTrue;
				if (!solver->mustBeTrue(data::SINGLE_ADDRESS_RESOLUTION, state, UgeExpr::create(address, mo->getBaseExpr()), mustBeTrue))
					return false;
				if (mustBeTrue)
					break;
			}
		...
		for (oi = start; oi != end; ++oi) {
			...
			bool mustBeTrue;
			if (!solver->mustBeTrue(data::SINGLE_ADDRESS_RESOLUTION, state, UltExpr::create(address, mo->getBaseExpr()), mustBeTrue))
				return false;
			if (mustBeTrue) {
				break;
			} else {
...
bool AddressSpace::resolve(ExecutionState &state, TimingSolver *solver, klee::ref<Expr> p, ResolutionList &rl, unsigned maxResolutions, double timeout) {
	if (ConstantExpr *CE = (dyn_cast<ConstantExpr>(p))) {
		...
		while (oi != begin) {
			...
			bool mustBeTrue;
			if (!solver->mustBeTrue(data::MULTI_ADDRESS_RESOLUTION, state, UgeExpr::create(p, mo->getBaseExpr()), mustBeTrue))
				return true;
			if (mustBeTrue)
				break;
		}
		...
		for (oi = start; oi != end; ++oi) {
			...
			bool mustBeTrue;
			if (!solver->mustBeTrue(data::MULTI_ADDRESS_RESOLUTION, state, UltExpr::create(p, mo->getBaseExpr()), mustBeTrue))
				return true;
			if (mustBeTrue)
				break;
...
void AddressSpace::DumpMemoryObject(std::ostream &os, const MemoryObject *mo, const ObjectState *ostate, bool fast) const {
...
void AddressSpace::DumpContents(std::ostream &os) const {
	...
	os << "Largest " << largest_count << " memory blocks:" << std::endl;

	for (int i = 0; i < largest_count; i++) {
		DumpMemoryObject(os, memory_vector[i], objects.find(memory_vector[i])->second, false);
	}
