...
namespace klee {
  ...
  public:
    ...
    void processMemoryLocation(ExecutionState &state,
        klee::ref<Expr> address, klee::ref<Expr> size,
        const std::string &name, resolutions_ty &resList);

    void makeSymbolic(ExecutionState &state, std::vector<klee::ref<Expr> > &arguments);

    bool writeConcreteValue(ExecutionState &state,
        klee::ref<Expr> address, uint64_t value, Expr::Width width);
...
#define HANDLER(name) void name(ExecutionState &state, \
    ...
    // For tapPLC, Daniel.
    HANDLER(handleTaskPriority);
    HANDLER(handleTaskPeriod);
    HANDLER(handleTaskBoundary);
    HANDLER(handleTaskBoundaryStart);
    HANDLER(handleTaskBoundaryEnd);
    HANDLER(handleScanStart);
    HANDLER(handleScanEnd);
    // End tapPLC
