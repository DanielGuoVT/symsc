...
cl::opt<bool> UseInterleavedRS("use-interleaved-RS");
...
cl::opt<bool> UseNonUniformRandomSearch("use-non-uniform-random-search");

cl::opt<bool> UseRandomPathSearch("use-random-path");

cl::opt<WeightedRandomSearcher::WeightType> WeightType("weight-type",
...
cl::opt<unsigned> BatchInstructions("batch-instructions",
		cl::desc(
				"Number of instructions to batch when using --use-batching-search"),
		cl::init(10000));

cl::opt<double> BatchTime("batch-time",
...
bool klee::userSearcherRequiresMD2U() {
	return (WeightType == WeightedRandomSearcher::MinDistToUncovered
			...
			|| UseInterleavedMD2UNURS || UseInterleavedCovNewNURS
			|| UseInterleavedInstCountNURS || UseInterleavedCPInstCountNURS
...
Searcher *klee::constructUserSearcher(Executor &executor, Searcher *original) {
	...
	if (UseInterleavedNURS || UseInterleavedMD2UNURS || UseInterleavedRS
		...
		if (UseInterleavedNURS)
			s.push_back(
					new WeightedRandomSearcher(executor,
