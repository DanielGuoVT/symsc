...
klee::ref<klee::ConstantExpr> Executor::toConstant(ExecutionState &state, klee::ref<Expr> e, const char *reason) {
	...
	os << "silently concretizing (reason: " << reason << ") expression " << e << " to value " << value << " (" << (*(state.pc())).info->file << ":"
			<< (*(state.pc())).info->line << ")";
...
klee::ref<Expr> Executor::toUnique(const ExecutionState &state, klee::ref<Expr> &e) {
	...
	if (!isa<ConstantExpr>(e)) {
		...
		if (solver->getValue(data::CHECK_UNIQUENESS, state, e, value)
				&& solver->mustBeTrue(data::CHECK_UNIQUENESS, state, EqExpr::create(e, value), isTrue) && isTrue)
			result = value;
		solver->setTimeout(0);
	}
...
klee::ref<ConstantExpr> Executor::evalConstantExpr(const llvm::ConstantExpr *ce) {
	...
	case Instruction::GetElementPtr: {
		...
		for (gep_type_iterator ii = gep_type_begin(ce), ie = gep_type_end(ce); ii != ie; ++ii) {
			klee::ref<ConstantExpr> addend = ConstantExpr::alloc(0, Context::get().getPointerWidth());

			if (StructType * st = dyn_cast<StructType>(*ii)) {
				const StructLayout *sl = kmodule->targetData->getStructLayout(st);
				const ConstantInt *ci = cast<ConstantInt>(ii.getOperand());

				addend = ConstantExpr::alloc(sl->getElementOffset((unsigned) ci->getZExtValue()), Context::get().getPointerWidth());
			} else {
				const SequentialType *set = cast<SequentialType>(*ii);
				klee::ref<ConstantExpr> index = evalConstant(cast<Constant>(ii.getOperand()));
				unsigned elementSize = kmodule->targetData->getTypeStoreSize(set->getElementType());
...
klee::ref<klee::ConstantExpr> Executor::evalConstant(const Constant *c) {
	if (getWidthForLLVMType(c->getType()) == 0) {
		...
		for (Constant::const_use_iterator it = c->use_begin(), ie = c->use_end(); it != ie; it++) {
	...
	if (const llvm::ConstantExpr *ce = dyn_cast<llvm::ConstantExpr>(c)) {
		...
		if (const ConstantInt *ci = dyn_cast<ConstantInt>(c)) {
			...
			if (cs->getNumOperands() == 0)
				return Expr::createPointer(0);
			klee::ref<klee::ConstantExpr> result = evalConstant(cs->getOperand(0));
			for (unsigned k = 1, e = cs->getNumOperands(); k != e; ++k) {
				klee::ref<klee::ConstantExpr> next = evalConstant(cs->getOperand(k));
			...
			if (cv->getNumOperands() == 0)
				return Expr::createPointer(0);
			klee::ref<klee::ConstantExpr> result = evalConstant(cv->getOperand(0));
			for (unsigned k = 1, e = cv->getNumOperands(); k != e; ++k) {
				klee::ref<klee::ConstantExpr> next = evalConstant(cv->getOperand(k));
				result = next->Concat(result);
			}
			return result;
		} else {
...
const Cell& Executor::eval(KInstruction *ki, unsigned index, ExecutionState &state) const {
	LOG_IF(INFO, index >= ki->inst->getNumOperands())
			<< "Line: " << ki->info->assemblyLine << " Index: " << index << " Num. op.: " << ki->inst->getNumOperands();
