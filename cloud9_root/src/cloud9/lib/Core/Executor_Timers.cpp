	...
	void run() {
		...
		if (executor->flog) {
			if (executor->precise) {
				...
				std::cerr << executor->inter;
				freopen("./log.Test", "w", stderr);
				std::cerr << executor->test;
			} else {
				freopen("./log.Inter", "w", stderr);
				std::cerr << executor->inter;
				freopen("./log.Fail", "w", stderr);
				if (executor->fail == 0 && executor->test == 0 && executor->inter > 0) {
					executor->fail = executor->inter;
				}
				std::cerr << executor->fail;
				freopen("./log.Test", "w", stderr);
				std::cerr << executor->test;
			}
			freopen("./log.maxaccs", "w", stderr);
			std::cerr << executor->maxMemAccs;
			fclose(stderr);
		}
	}
};
...
public:
	TimerInfo(Timer *_timer, double _rate) :
			timer(_timer), rate(_rate), nextFireTime(util::getWallTime() + rate) {
	}
	~TimerInfo() {
		delete timer;
	}
