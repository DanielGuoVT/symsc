...
UpdateNode::UpdateNode(const UpdateNode *_next, const klee::ref<Expr> &_index, const klee::ref<Expr> &_value) :
...
UpdateNode::~UpdateNode() {
	// XXX gross
	if (stpArray) ::vc_DeleteExpr(stpArray);
}

int UpdateNode::compare(const UpdateNode &b) const {
	if (int i = index.compare(b.index)) return i;
...
int UpdateList::compare(const UpdateList &b) const {
	if (root->name != b.root->name) return root->name < b.root->name ? -1 : 1;
	...
	for (; an && bn; an = an->next, bn = bn->next) {
		if (an == bn) { // exploit shared list structure
			...
			if (int res = an->compare(*bn)) return res;
		}
	}
	assert(!an && !bn);
	return 0;
...
unsigned UpdateList::hash() const {
	...
	for (unsigned e = root->name.size(); i != e; ++i)
