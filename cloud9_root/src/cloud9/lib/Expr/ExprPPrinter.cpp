...
private:
	...
	bool isSimple(const klee::ref<Expr> &e) {
		bool verySimple = isVerySimple(e);
		if (verySimple) {
			return true;
			...
			for (unsigned i = 0; i < ep->getNumKids(); i++)
				if (!isVerySimple(ep->getKid(i))) return false;
			return true;
		}
	}
...
//			// sjguo
//			if (e->getKind() == Expr::WPLoad || e->getKind() == Expr::WPRegular) {
//				return;
//			}
			//end
	...
	void printUpdateList(const UpdateList &updates, PrintContext &PC) {
		...
		for (const UpdateNode *un = head; un; un = un->next) {
			...
			if (it != updateBindings.end()) {
				...
				if (openedList) PC << "] @";
				if (un != head) PC.breakLine(outerIndent);
				PC << "U" << updateCounter << ":";
	...
	void printWidth(PrintContext &PC, klee::ref<Expr> e) {
		if (!shouldPrintWidth(e)) return;

		if (PCWidthAsArg) {
			PC << ' ';
			if (PCPrefixWidth) PC << 'w';
		}
	...
	const ReadExpr* hasOrderedReads(klee::ref<Expr> e, int stride) {
		...
		if (stride == -1) return cast<ReadExpr>(e.get());
		else return base;
	}
...
//		if (ep->getKind() == Expr::WPRegular) {
//			assert(ep->getNumKids() == 0 && "number of kinds of WPExpr should be 0");
//			WPRegularExpr *we = (WPRegularExpr*) ep;
////			PC << "name ";
//			PC << we->getName();
//			PC << " ";
//			PC << "tid ";
...
//			PC << " ";
//			return;
//		} else if (ep->getKind() == Expr::WPLoad) {
//			assert(ep->getNumKids() == 1 && "number of kinds of WPExpr should be 0");
//			WPLoadExpr *we = (WPLoadExpr*) ep;
////			PC << "name ";
//			PC << we->getName();
//			PC << " ";
//			PC << "tid ";
...
public:
	...
	void printConst(const klee::ref<ConstantExpr> &e, PrintContext &PC, bool printWidth) {
		if (e->getWidth() == Expr::Bool) PC << (e->isTrue() ? "true" : "false");
		else {
			if (PCAllConstWidths) printWidth = true;

			if (printWidth) PC << "(w" << e->getWidth() << " ";
		...
		if (ConstantExpr * CE = (dyn_cast<ConstantExpr>(e))) {
			...
			if (it != bindings.end()) {
				...
				if (PCMultibyteReads && e->getKind() == Expr::Concat) {
					...
					int isLSB = (base != NULL);
					if (!isLSB) base = hasOrderedReads(e, 1);
					if (base) {
	...
	void printSeparator(PrintContext &PC, bool simple, unsigned indent) {
		if (simple) {
			...
			// to avoid multiple breaklines, daniel.
//			PC.breakLine(indent);
			PC << ' ';
		}
	}
...
void ExprPPrinter::printOne(std::ostream &os, const char *message, const klee::ref<Expr> &e) {
	PPrinter p(os);
	p.scan(e);
...
void ExprPPrinter::printConstraints(std::ostream &os, const ConstraintManager &constraints) {
	printQuery(os, constraints, ConstantExpr::alloc(false, Expr::Bool), std::string(), std::string());
}

void ExprPPrinter::printQuery(std::ostream &os, const ConstraintManager &constraints, const klee::ref<Expr> &q, const std::string &parentID,
		const std::string &queryID, const klee::ref<Expr> *evalExprsBegin, const klee::ref<Expr> *evalExprsEnd, const Array * const *evalArraysBegin,
	...
	if (evalExprsBegin != evalExprsEnd) {
		...
		for (const klee::ref<Expr> *it = evalExprsBegin; it != evalExprsEnd; ++it) {
			...
			if (it + 1 != evalExprsEnd) PC.breakLine(indent);
		}
		PC << ']';
	}
	...
	if (evalArraysBegin != evalArraysEnd) {
		if (evalExprsBegin == evalExprsEnd) PC << " []";

		PC.breakLine(indent - 1);
		PC << '[';
		for (const Array * const * it = evalArraysBegin; it != evalArraysEnd; ++it) {
			PC << (*it)->name;
			if (it + 1 != evalArraysEnd) PC.breakLine(indent);
		}
		PC << ']';
	}
