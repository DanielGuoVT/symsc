...
klee::ref<Expr> ExprVisitor::visitActual(const klee::ref<Expr> &e) {
	if (isa<ConstantExpr>(e)) {
		...
		case Expr::Sge:
			...
			break;
//		case Expr::WPRegular:
//			res = visitWPRegular(static_cast<WPRegularExpr&>(ep));
//			break;
//		case Expr::WPLoad:
//			res = visitWPLoad(static_cast<WPLoadExpr&>(ep));
//			break;
		case Expr::Constant:
		default:
		...
		case Action::DoChildren: {
			...
			for (unsigned i = 0; i < count; i++) {
				...
				kids[i] = visit(kid);
				if (kids[i] != kid) rebuild = true;
			}
			if (rebuild) {
				e = ep.rebuild(kids);
				if (recursive) e = visit(e);
			}
			if (!isa<ConstantExpr>(e)) {
				res = visitExprPost(*e.get());
				if (res.kind == Action::ChangeTo) e = res.argument;
			}
			return e;
		}
		case Action::SkipChildren:
...
//ExprVisitor::Action ExprVisitor::visitWPRegular(const WPRegularExpr&) {
//	return Action::skipChildren();
//}
//ExprVisitor::Action ExprVisitor::visitWPLoad(const WPLoadExpr&) {
//	return Action::doChildren();
//}
