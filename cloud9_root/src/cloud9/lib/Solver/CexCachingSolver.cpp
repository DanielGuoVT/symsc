...
bool CexCachingSolver::computeValidity(const Query& query, Solver::Validity &result) {
	...
	if (a){
		...
		std::cerr << "query.expr: " << query.expr << std::endl;
		std::cerr << "query.withFalse().expr: " << query.withFalse().expr << std::endl;
	}
...
bool CexCachingSolver::computeValue(const Query& query, klee::ref<Expr> &result) {
	TimerStatIncrementer t(stats::cexCacheTime);
