...
static void __add_symfs_file(fs_init_descriptor_t *fid, enum sym_file_type file_type, const char *file_path) {
...
void klee_process_args(int* argcPtr, char*** argvPtr) {
	...
	while (k < argc) {
		if (__streq(argv[k], "--sym-arg") || __streq(argv[k], "-sym-arg")) {
			...
			__add_arg(&new_argc, new_argv, __get_sym_str(max_len, sym_arg_name), 1024);
		} else if (__streq(argv[k], "--sym-args") || __streq(argv[k], "-sym-args")) {
			const char *msg = "--sym-args expects three integer arguments <min-argvs> <max-argvs> <max-len>";
