...
cl::list<std::string> ReadArgsFilesFake("read-args", cl::desc("File to read arguments from (one arg per line)"));
...
cl::opt<bool> Watchdog("watchdog", cl::desc("Use a watchdog process to enforce --max-time."), cl::init(0));
}
...
int main(int argc, char **argv, char **envp) {
	...
	Interpreter *interpreter = theInterpreter = Interpreter::create(IOpts, handler);
	...
	const Module *finalModule = interpreter->setModule(mainModule, mOpts);
	...
	if (queries)
		handler->getInfoStream() << "KLEE: done: avg. constructs per query = " << queryConstructs / queries << "\n";
