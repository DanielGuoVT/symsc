...
* MISTY1 FI Function
*/
uint16_t input, key7, key9;
uint16_t D9, D7, Result;
void FI()
{
   D9 = input >> 7;
   D7 = input & 0x7F;
...
void encrypt()
{
  ...
  for(i = 0; i != BLOCK_NUM; ++i)
  {
    ...
    p = (in + 1 * sizeof(uint16_t));
    for (j = 0; j != sizeof(uint16_t); ++j){
      B1 = (B1 << 8) | p[j];
    }

    p = (in + 2 * sizeof(uint16_t));
    for (j = 0; j != sizeof(uint16_t); ++j){
      B2 = (B2 << 8) | p[j];
    }

    p = (in + 3 * sizeof(uint16_t));
    for (j = 0; j != sizeof(uint16_t); ++j){
      B3 = (B3 << 8) | p[j];
    } 
