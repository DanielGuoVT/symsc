...
uint8_t in[CAST5_BLOCK_SIZE] = {0x0d,0x77,0x1f,0xa9,0x8b,0x0c,0x1c,0x2b};
uint8_t out[CAST5_BLOCK_SIZE] = {0};
uint8_t in_key[CAST5_KEY_SIZE] = {0};

void encrypt()
{
  static const uint32_t *src = (const uint32_t *)in;
  static uint32_t *dst = (uint32_t *)out;
  static uint32_t l, r, t;
  static uint32_t I;     /* used by the Fx macros */
  static uint32_t *Km;
  static uint8_t *Kr;
...
int setkey()
{
  ...
  static uint32_t *p_key = (uint32_t *)in_key;

  ctx.rr = CAST5_KEY_SIZE <= 10 ? 1 : 0;
...
#define zi(i)   ((z[(i)/4] >> (8*(3-((i)%4)))) & 0xff)
  z[0] = x[0] ^ s5[xi(13)] ^ s6[xi(15)] ^ s7[xi(12)] ^ sb8[xi(14)] ^ s7[xi(8)];
...
#define shadow
#ifdef shadow
uint8_t in_key_shadow[CAST5_KEY_SIZE] = {0};
...
int setkey1()
{
  ...
  static uint32_t z[4];
  static uint32_t k[16];
  static uint32_t *p_key = (uint32_t *)in_key_shadow;
...
int main(void)
{
  ...
  #define shadow
  #ifdef shadow
  klee_make_symbolic(in_key_shadow, sizeof(in_key_shadow), "in_key_shadow");
  klee_task_boundary();
  setkey1();
  encrypt1();
  #endif
