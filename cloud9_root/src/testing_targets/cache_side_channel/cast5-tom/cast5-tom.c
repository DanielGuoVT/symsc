...
block_state sched;
uint8 in_key[16] = { 0x01, 0x23, 0x45, 0x67, 0x12, 0x34, 0x56, 0x78, 0x23, 0x45, 0x67, 0x89, 0x34, 0x56, 0x78, 0x9A };
uint8 in[8] = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF };
uint8 out[8] =  { 0x23, 0x8B, 0x4F, 0xE5, 0x84, 0x7E, 0x44, 0xB2 };
...
static void castcrypt()
{
	static uint32 L, R, tmp, f;
	static uint32 Kmi;
	static uint8  Kri;
	static short functype, round;
	...
	for(round = 0; round < sched.rounds; round ++) {
		...
		if (!decrypt) {
			...
			Kmi = sched.Km[(sched.rounds) - round - 1];
			Kri = sched.Kr[(sched.rounds) - round - 1];
			functype = (((sched.rounds) - round - 1) % 3);
		}
	
		/* this is the round function f(D, Km, Kr) */
		static uint32 I;
		static short Ia, Ib, Ic, Id;
	...
	out[0] = ( R & 0xFF000000 ) >> 24;
	out[1] = ( R & 0x00FF0000 ) >> 16;
	out[2] = ( R & 0x0000FF00 ) >> 8;
	out[3] = ( R & 0x000000FF );
	out[4] = ( L & 0xFF000000 ) >> 24;
	out[5] = ( L & 0x00FF0000 ) >> 16;
	out[6] = ( L & 0x0000FF00 ) >> 8;
	out[7] = ( L & 0x000000FF );
}
...
#define zxround1() XZRound(z1, tmp1, 0, 2, 3, 1, \
			b(tmp1,13), b(tmp1,15), b(tmp1,12), b(tmp1,14),\
			b(tmp1, 8), b(tmp1,10), b(tmp1, 9), b(tmp1,11))

#define xzround1() XZRound(tmp1, z1, 2, 0, 1, 3, \
			b(z1,5), b(z1,7), b(z1,4), b(z1,6), \
			b(z1,0), b(z1,2), b(z1,1), b(z1,3))

#define zxround2() XZRound(z2, tmp2, 0, 2, 3, 1, \
			b(tmp2,13), b(tmp2,15), b(tmp2,12), b(tmp2,14),\
			b(tmp2, 8), b(tmp2,10), b(tmp2, 9), b(tmp2,11))

#define xzround2() XZRound(tmp2, z2, 2, 0, 1, 3, \
			b(z2,5), b(z2,7), b(z2,4), b(z2,6), \
			b(z2,0), b(z2,2), b(z2,1), b(z2,3))
...
static void schedKeys()
{
	static uint32 x[4];
	static uint8  padkey[16];
	static uint32 Kr_wide[16];
	static int i;
	...
	/* generates sixteen 32-bit subkeys based on a 4x32-bit input; modifies the x as well. */
	static uint32 tmp1[4], z1[4];
	...
	zxround1();
	Kround(sched.Km, 0, z1,
	       8,  9, 7, 6,  2,
	       10, 11, 5, 4,  6,
	       12, 13, 3, 2,  9,
	       14, 15, 1, 0, 12);
	xzround1();
	Kround(sched.Km, 4, tmp1,
	       3,  2, 12, 13,  8,
	       1,  0, 14, 15, 13,
	       7,  6,  8,  9,  3,
	       5,  4, 10, 11,  7);
	zxround1();
	Kround(sched.Km, 8, z1,
	       3,  2, 12, 13,  9,
	       1,  0, 14, 15, 12,
	       7,  6,  8,  9,  2,
	       5,  4, 10, 11,  6);
	xzround1();
	Kround(sched.Km, 12, tmp1,
	       8,  9, 7, 6,  3,
	       10, 11, 5, 4,  7,
	       12, 13, 3, 2,  8,
	       14, 15, 1, 0, 13);
	...
	for(i = 0; i < KEY_SIZE; i ++) {
		...
		sched.Kr[i] = (uint8)(Kr_wide[i] & 0x1F);
	}
}
...
#ifdef shadow
uint8 in_key_shadow[16] = { 0x01, 0x23, 0x45, 0x67, 0x12, 0x34, 0x56, 0x78, 0x23, 0x45, 0x67, 0x89, 0x34, 0x56, 0x78, 0x9A };

static void schedKeys1()
{
   static uint32 x[4];
   static uint8  padkey[16];
   static uint32 Kr_wide[16];
...
int64_t dummy;
void thread2(){
  dummy = 0x0000ffd011cd7cfd;
}

int main(void)
{
  /* Single Plaintext-Key-Ciphertext Sets */
