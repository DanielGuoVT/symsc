...
#define rotate_left(input,rot) (input<<rot)|(input>>(8*sizeof(input)-rot))
#define rotate_right(input,rot) (input>>rot)|(input<<(8*sizeof(input)-rot))

uint16_t EK[64] = {0};
uint8_t key[KEY_SIZE]={0x11,0x7f,0xde,0x6e,0x33,0xb8,0xff,0xcc,0xcd,0x66,0xab,0xfb,0xcf,0x8b,0x54,0xf3};
uint8_t in[BLOCK_SIZE]={0x34,0xf4,0xdd,0xbe,0xfb,0xc7,0x1f,0x2d,0x3d,0xf8,0xaf,0xed,0x2e,0xdf,0x38,0x8f};
uint8_t out[BLOCK_SIZE]={0};

 /*
* KASUMI FI Function
*/
uint16_t I=0, KK=0, Result=0;
uint16_t D9, D7;
...
void FI()
{
   ...
   D7 ^= (KK >> 9);
   D9 = KASUMI_SBOX_S9[D9 ^ (KK & 0x1FF)] ^ D7;
   D7 = KASUMI_SBOX_S7[D7] ^ (D9 & 0x7F);
   Result = (D7 << 9) | D9;
}
...
void encrypt()
{
  static uint16_t *p;
  static unsigned i, j;
  static uint16_t B0 = 0;
  static uint16_t B1 = 0;
  static uint16_t B2 = 0;
  static uint16_t B3 = 0;

  p = (uint16_t *)in;

  for(i = 0; i != BLOCK_NUM; ++i)
  {
    // p = in + 0 * sizeof(uint16_t);
    for (j = 0; j != sizeof(uint16_t); ++j){
      B0 = (B0 << 8) | p[j];
    }

    p = (uint16_t *)(in + 1 * sizeof(uint16_t));
    for (j = 0; j != sizeof(uint16_t); ++j){
      B1 = (B1 << 8) | p[j];
    }    

    p = (uint16_t *)(in + 2 * sizeof(uint16_t));
    for (j = 0; j != sizeof(uint16_t); ++j){
      B2 = (B2 << 8) | p[j];
    }    
    
    p = (uint16_t *)(in + 3 * sizeof(uint16_t));
    for (j = 0; j != sizeof(uint16_t); ++j){
      B3 = (B3 << 8) | p[j];
    }
    ...
    for(j = 0; j != 8; j += 2)
    {
      const uint16_t* K = &EK[8*j];

      static uint16_t R, L;
      R = B1 ^ (rotate_left(B0, 1) & K[0]);
      L = B0 ^ (rotate_left(R, 1) | K[1]);
...
#define shadow
#ifdef shadow
uint8_t key_shadow[KEY_SIZE]={0x11,0x7f,0xde,0x6e,0x33,0xb8,0xff,0xcc,0xcd,0x66,0xab,0xfb,0xcf,0x8b,0x54,0xf3};
void key_schedule1()
{
  static const uint16_t RC[] = { 
    0x0123, 0x4567, 0x89AB, 0xCDEF,0xFEDC, 0xBA98, 0x7654, 0x3210 };
  ...
  static uint16_t tmp = 0;
  static uint8_t *p;
  p = (uint8_t *)key_shadow;
  
  for(i = 0; i != 8; ++i)
  {
    tmp = 0;
    p = key_shadow + i * sizeof(uint16_t);
    for (j = 0; j != sizeof(uint16_t); ++j){
      tmp = (tmp << 8) | p[j];
    }
    K[i] = tmp;
