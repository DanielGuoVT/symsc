 ...
 * or D_i block bits 7-1 (bit 0 being the least significant).
 */
...
struct des_ctx ctx;
uint8_t block[DES_BLOCK_SIZE] = {0xff, 0x0e, 0xf1, 0xf0, 0x3f, 0x4d, 0xf5, 0x55};
uint8_t out[DES_BLOCK_SIZE] = {0};
uint8_t key[DES_KEY_SIZE] = {0xff, 0x0e, 0xf1, 0xf0, 0x3f, 0x4d, 0xf5, 0x55};
uint32_t pe[DES_EXPKEY_WORDS]; 
 ...
 *   alternating zero-one patterns block the C and D registers after Permuted
...
void dkey()
{
  ...
  d = key[4]; d &= 0x0e; d <<= 4; d |= key[0] & 0x1e; d = pc1[d];
  c = key[5]; c &= 0x0e; c <<= 4; c |= key[1] & 0x1e; c = pc1[c];
  b = key[6]; b &= 0x0e; b <<= 4; b |= key[2] & 0x1e; b = pc1[b];
  a = key[7]; a &= 0x0e; a <<= 4; a |= key[3] & 0x1e; a = pc1[a];
  ...
  d = key[0]; d &= 0xe0; d >>= 4; d |= key[4] & 0xf0; d = pc1[d + 1];
  c = key[1]; c &= 0xe0; c >>= 4; c |= key[5] & 0xf0; c = pc1[c + 1];
  b = key[2]; b &= 0xe0; b >>= 4; b |= key[6] & 0xf0; b = pc1[b + 1];
  a = key[3]; a &= 0xe0; a >>= 4; a |= key[7] & 0xf0; a = pc1[a + 1];
...
//   }
  ...
  /* Copy to output */
  memcpy((&ctx)->expkey, pe, sizeof((&ctx)->expkey));
...
void encrypt()
{
  static const uint32_t *K = (&ctx)->expkey;
  static const uint32_t *s = (const uint32_t *)block;
  static uint32_t *d = (uint32_t *)out;
  static uint32_t L, R, A, B;
  static int i;
...
void decrypt()
{
  static const uint32_t *K = (&ctx)->expkey + DES_EXPKEY_WORDS - 2;
  static const uint32_t *s = (const uint32_t *)block;
  static uint32_t *d = (uint32_t *)out;
  static uint32_t L, R, A, B;
  static int i;
...
#define shadow
#ifdef shadow
uint8_t key_shadow[DES_KEY_SIZE] = {0xff, 0x0e, 0xf1, 0xf0, 0x3f, 0x4d, 0xf5, 0x55};
